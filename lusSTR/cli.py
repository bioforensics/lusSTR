...
def annot_subparser(subparsers):
    ...
    cli.add_argument(
        "input",
        help="sample(s) in CSV format; first four columns must be Locus, NumReads, "
        "Sequence, SampleID; Optional last two columns can be Project and Analysis.",
    )
    cli.add_argument(
        "--kit",
        choices=["forenseq", "powerseq"],
        default="forenseq",
...
def filter_subparser(subparsers):
    ...
    cli.add_argument("input", help="Input is a single lusSTR output file (.txt format)")
    cli.add_argument(
        "--separate",
        action="store_true",
