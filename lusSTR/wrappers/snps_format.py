...
def straitrazor_filtering(sr_df):
    for i, row in sr_df.iterrows():
        ...
        if sr_df.loc[i, "Issues"] == "Allele call does not match expected allele!":
            ...
            if allele_reads < (total_snp_reads * 0.015):
                sr_df.loc[i, "Reads"] = 0
    return sr_df


def main(input, output, kit, strand, separate, refs, uas):
    input = str(input)
    ...
    if uas:
        ...
        results = straitrazor_filtering(input_file)
